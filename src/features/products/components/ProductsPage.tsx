...
export function ProductsPage() {
  ...
  const { filters, sort, status, error, preferencesRestored } =
  ...
  useEffect(() => {
    if (preferencesRestored) {
      saveProductPreferences({ filters, sort });
    }
  }, [filters, preferencesRestored, sort]);

  function handleSearch(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    dispatch(setQuery(filters.query.trim()));
  }
        ...
        <div className="filter-grid">
          ...
          <label className="control">
            ...
            <input
              ...
              step="1"
              placeholder={
                stats.highestPrice ? `$${Math.ceil(stats.highestPrice)}` : "$0"
              }
          ...
          <label className="control">
            ...
            <select
              ...
              onChange={(event) => dispatch(setMinRating(event.target.value))}
            >
          ...
          <label className="control">
            ...
            <select
              value={sort}
              onChange={(event) =>
                dispatch(setSort(event.target.value as ProductSort))
              }
            >
